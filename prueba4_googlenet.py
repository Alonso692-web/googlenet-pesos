...
# --- Interfaz Tkinter ---
BASE_FONT_SIZE = 22  # Aumentado el tamaño base de la fuente
INITIAL_SCREEN_WIDTH = 900  # Aumentado para más espacio
INITIAL_SCREEN_HEIGHT = 700  # Aumentado para más espacio vertical

root = tk.Tk()
root.title('Clasificador de Etapas de Crecimiento')
root.geometry(f"{INITIAL_SCREEN_WIDTH}x{INITIAL_SCREEN_HEIGHT}")
root.minsize(700, 500)  # Tamaño mínimo mayor

# Grid principal (root) se expande
root.grid_rowconfigure(0, weight=1)
root.grid_columnconfigure(0, weight=1)

main_frame = tk.Frame(root, padx=8, pady=8)  # Más padding
...
left_frame.grid(row=0, column=0, sticky="nsew", padx=(0, 8))  # Más padding
left_frame.grid_rowconfigure(0, weight=1)
left_frame.grid_columnconfigure(0, weight=1)

image_label = tk.Label(left_frame, text='Imagen', fg='gray', 
                      font=font.Font(size=BASE_FONT_SIZE + 8, weight='bold'))  # Fuente más grande y en negrita
...
right_frame.grid(row=0, column=1, sticky="nsew", padx=(8, 0))  # Más padding

# Configurar filas para el nuevo diseño
right_frame.grid_rowconfigure(0, weight=0)  # Etiqueta de etapa (no se expande)
right_frame.grid_rowconfigure(1, weight=1)  # Descripción (se expande)
right_frame.grid_rowconfigure(2, weight=0)  # Botones (no se expande)
right_frame.grid_columnconfigure(0, weight=1)

# Etiqueta para mostrar la etapa clasificada (en negrita y más grande)
stage_label = tk.Label(right_frame, text='', font=font.Font(
    size=BASE_FONT_SIZE + 4, weight='bold'), fg='blue', anchor='nw', justify=tk.LEFT)
stage_label.grid(row=0, column=0, sticky="nw", padx=15, pady=(15, 5))

# Etiqueta para mostrar la descripción
status_label = tk.Label(right_frame, text='Esperando acción...', 
...
button_frame.grid(row=2, column=0, sticky="sew", padx=5, pady=10)  # Más padding
button_frame.grid_columnconfigure(0, weight=1)  # La columna de botones se expande horizontalmente

# Fuentes más grandes para botones
capture_btn_font = font.Font(size=BASE_FONT_SIZE, weight='bold')
clear_btn_font = font.Font(size=BASE_FONT_SIZE)

capture_btn = tk.Button(button_frame, text='Tomar Foto', font=capture_btn_font, 
                        command=lambda: tomar_y_clasificar(), height=2)
capture_btn.grid(row=0, column=0, sticky="ew", pady=(0, 8))  # Más espacio

clear_btn = tk.Button(button_frame, text='Limpiar', font=clear_btn_font, 
                      command=lambda: limpiar(), state=tk.DISABLED, height=2)
...
def limpiar():
    ...
    image_label.config(image=None, text='Imagen', fg='gray', font=font.Font(size=BASE_FONT_SIZE + 8, weight='bold'))
...
def update_status_wraplength(event=None):
    ...
    new_width = right_frame.winfo_width() - 30  # Más margen para fuentes grandes
